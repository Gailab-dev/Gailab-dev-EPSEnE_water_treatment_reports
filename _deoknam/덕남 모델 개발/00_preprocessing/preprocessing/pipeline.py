...
D(적산계): diff<0 글리치(짧은 원복)만 NaN → ffill
마지막에 전처리 완료된 전중염소를 전염소:중염소 = 1:10으로 분할.
"""
import numpy as np
import pandas as pd
...
def clean_setpoint_column(s: pd.Series, col: str):
    ...
    설정치는 값 유지가 의미상 타당하므로 결측 길이 캡 미적용(플래그만 생성).
    """
    clean, phys_bad = _phys_clip(s, col)
...
def clean_accum_column(s: pd.Series, col: str):
    """D 그룹: 적산계. 음수 및 짧게 원복되는 diff<0 글리치만 NaN → ffill.

    적산계는 단조증가라 값 유지(ffill)가 타당하므로 결측 길이 캡 미적용(플래그만 생성).
    """
    clean, phys_bad = _phys_clip(s, col)
    d = clean.diff()
    drop = (d < 0).fillna(False)
    glitch = pd.Series(False, index=s.index)
    drop_pos = np.flatnonzero(drop.to_numpy())
    vals = clean.to_numpy()
    for i in drop_pos:
        # i 시점에서 떨어졌다가 ACCUM_GLITCH_SPAN 내 직전 수준 이상으로 원복 → 글리치
        prev = vals[i - 1]
        span = vals[i:min(i + C.ACCUM_GLITCH_SPAN + 1, len(vals))]
        if np.any(span >= prev):
            end = i + int(np.argmax(span >= prev))
            glitch.iloc[i:end] = True
    clean[glitch] = np.nan
    pre_fill_nan = clean.isna().to_numpy().copy()
    clean = clean.ffill(limit_area="inside")
    fill_flag = pd.Series(pre_fill_nan & ~clean.isna().to_numpy(), index=s.index)
    removed = phys_bad | glitch
    counts = {"물리범위밖": int(phys_bad.sum()), "Hampel": 0, "IQR": 0,
              "ISO원탐지": 0, "ISO채택": int(glitch.sum()), "최종제거": int(removed.sum()),
              "장기결측유지": 0}
    return clean, removed, counts, fill_flag


def split_chlorine(df: pd.DataFrame) -> pd.DataFrame:
    """전처리 완료된 전중염소를 전염소 1 : 중염소 10 비율로 분할한 컬럼 추가."""
    for parent, (pre_col, mid_col) in C.CHLORINE_SPLIT.items():
        df[pre_col] = df[parent] * C.PRE_RATIO
        df[mid_col] = df[parent] * C.MID_RATIO
    return df


def run_pipeline(df: pd.DataFrame, verbose: bool = True):
    """전 컬럼 그룹 디스패치 → 전중염소 분할 → 리포트.
    ...
    for col in df.columns:
        ...
        if grp == "A.수질센서":
            ...
            clean, removed, counts, fill_flag = clean_accum_column(s, col)
        df_clean[col] = clean
        masks[col] = removed
        flags[col] = fill_flag
        report.append({
    ...
    df_clean = split_chlorine(df_clean)
