# -*- coding: utf-8 -*-
"""추천용 탁도 모델: 주입률 강제 포함 + 단조 제약 XGBoost (침전지별 2모델).

타깃은 02_coag와 동일한 델타(평활탁도(t+lag) − 평활탁도(t)) 원단위.
트리 모델이므로 스케일링 없이 학습한다.
"""
import joblib
import numpy as np
import pandas as pd
...
def build_dataset(cid: int):
    ...
    df10, _ = features.mask_idle_basin2(df10)
...
def train_dose_models(table: pd.DataFrame):
    """train+val로 침전지별 단조제약 XGB 학습. 반환 {1: model, 2: model}."""
