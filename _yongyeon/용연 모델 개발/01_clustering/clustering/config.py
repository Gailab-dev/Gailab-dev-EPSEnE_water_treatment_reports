# -*- coding: utf-8 -*-
"""용연 원수성상 슬라이딩 윈도우 군집화 설정 (덕남 이식본).
...
# 경로: config.py → clustering → 01_clustering → 용연 모델 개발
ROOT = Path(__file__).resolve().parents[2]
IN_PARQUET = ROOT / "dataset" / "preprocessing_data" / "용연_전처리완료.parquet"
CLUSTER_DATA_DIR = ROOT / "dataset" / "clustering_data"
OUT_LABELED = CLUSTER_DATA_DIR / "용연_군집라벨.parquet"
OUTPUT_DIR = ROOT / "01_clustering" / "output"
PLOTS_DIR = OUTPUT_DIR / "plots"
MODEL_PATH = OUTPUT_DIR / "model" / "용연_kmeans_k3.joblib"

# 원수성상 5컬럼 — 순서가 특징 벡터 순서를 결정 (PLANT_REGISTRY와 일치)
RAW_FEATURES = [
    "원수 탁도",
    "원수 PH",
    "원수 알카리도",
