...
def main():
    # 현재 스크립트 위치:
    # _yongyeon/ml/00_preprocess_outlier
    ...
    # 출력 위치:
    # _yongyeon/ml/00_preprocess_outlier/output/...
    ...
    summary = run_outlier_pipeline(
        input_path=dataset_dir / "용연_응집제공정_소독공정_통합.parquet",
        output_path=output_dataset_dir / "용연_응집제공정_소독공정_통합_이상치제거.parquet",
        report_csv_path=output_report_dir / "용연_이상치제거_리포트.csv",
        report_md_path=output_report_dir / "용연_전처리_리포트.md",
        plant_name="용연",
    )

    save_status = "저장 완료" if summary.get("저장 여부", True) else "저장 생략"
    print(f"용연 이상치 제거 처리 완료 ({save_status})")
